...
class MavenScraper:
    ...
    def get_deps_dev_dependencies(self, system: str, pkg: str, version: str) -> List[str]:
        ...
        url = f"https://api.deps.dev/v3alpha/systems/{system}/packages/{self.url_encode(pkg)}/versions/{self.url_encode(version)}:dependencies"
        ...
        for i in range(0, len(purls), batch_size):
            ...
            for purl in batch:
                try:
                    ...
                    url = f"https://api.deps.dev/v3alpha/systems/{system}/packages/{self.url_encode(name)}/versions/{self.url_encode(version)}"
                    response = self.session.get(url)
    ...
    def scrape_mvn_and_output_json(self, output_file: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Summary: {len(all_packages)} popular packages, {len(dependencies_list)} dependencies, {len(github_repos)} GitHub repos")
            return True
