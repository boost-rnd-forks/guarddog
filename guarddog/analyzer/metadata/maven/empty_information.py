...
class MavenEmptyInfoDetector(EmptyInfoDetector):
    def detect(self, package_info, path: Optional[str] = None, name: Optional[str] = None,
        ...
        log.debug(f"Running Maven empty description heuristic on package {name} version {version}")
        return len(package_info["info"]["description"].strip()) == 0, EmptyInfoDetector.MESSAGE_TEMPLATE % "Maven"
