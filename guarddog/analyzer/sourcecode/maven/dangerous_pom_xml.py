...
class MavenDangerousPomXML(Detector):
    ...
    def detect(self, path: Optional[str] = None) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            effective_pom_path = ""
            log.error(f"Error with generating the effective pom path: {e}")
            return False, None
        message += "\nResults of the effective pom analysis.\n"
        ...
        for plugin, entries in suspicious_tags.items():
            for entry in entries:
                ...
                message += (
                    f"\t\tFound in <execution> : {tag} bound with early phase {phase}\n"
                )
                result = True

        urls_in_cmd: bool = self.url_as_cmd_argument(effective_pom_path)
        if urls_in_cmd:
            message += "\n\n\t* An URL was found to be used as a command argument in <argument> or <script>."
            result = True
    ...
    def get_effective_pom(self, path: str) -> str:
        ...
        """
        log.debug("Generating the effective pom...")
        if not os.path.isdir(path):
        ...
        try:
            ...
            if os.path.exists(effective_pom_path):
                ...
                log.debug("Failed to generate the effective pom")
    ...
    def http_unsafe(self, pom_path: str) -> tuple[bool, list]:
        ...
        root = tree.getroot()
        bad_urls = []
        log.debug("Scanning for insecure HTTP URLs...")
        found = False
    ...
    def dangerous_tags_combinations(self, pom_path: str) -> dict:
        ...
        If are found in an <execution> tag
            ...
            Return:
                dict(plugin_id: {phases: list, tag: (tag, cmd)})
        """
        tree = ET.parse(pom_path)
        root = tree.getroot()
        results: dict = {}
        log.debug("Scanning for dangerous tags ...")
        for plugin in root.findall(".//mvn:plugin", NAMESPACE):
            artifact_id = plugin.find("mvn:artifactId", NAMESPACE)
    ...
    def url_as_cmd_argument(self, pom_path: str) -> bool:
        ...
        found = False
        urls = ["http://", "https://", "ftp://"]
        log.debug("Scanning for urls as cmd args ...")
    ...
    def suspicious_argline_usage(self, pom_path: str) -> tuple[bool, list[str]]:
        ...
        suspicious = False
        suspicious_commands = []
        log.debug("Scanning for <argLine> commands...")
