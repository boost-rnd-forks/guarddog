...
class MavenPackageScanner(PackageScanner):
    ...
    def _get_path_for_artifact(
        self,
        package_name: str,
        version: str,
        classifier: typing.Optional[str] = None,
        extension: str = "jar",
    ) -> str:
    ...
    def _parse_pom(self, pom_path: str) -> dict:
        ...
        emails = []
        for dev in root.findall(f".//{{{namespace}}}developer"):
            email = dev.find(f".//{{{namespace}}}email")
        ...
        name = (
            ...
            else (artifact_id or "unknown")
        )

        return {
            "info": {"name": name, "version": version or "unknown", "email": emails}
        }
    ...
    def download_package(
        self, package_name: str, directory: str, version: typing.Optional[str] = None
    ) -> str:
        if version is None:
        ...
        sources_path_in_repo = self._get_path_for_artifact(
            package_name, version, classifier="sources", extension="jar"
        )
        ...
        if os.path.exists(full_sources_path):
            ...
            regular_path_in_repo = self._get_path_for_artifact(
                package_name, version, classifier=None, extension="jar"
            )
            ...
            if os.path.exists(full_regular_path):
                ...
                jar_type = "regular"
                log.debug(
                    f"Found regular JAR for {package_name}:{version} (sources not available)"
                )
            else:
                raise FileNotFoundError(
                    f"Could not find JAR for {package_name}:{version} in local Maven repository. "
                    f"Looked for sources at {full_sources_path} and regular at {full_regular_path}"
                )

        # Copy and extract the JAR
        destination_jar = os.path.join(
            directory, f"{artifact_id}-{version}-{jar_type}.jar"
        )
    ...
    def download_and_get_package_info(
        self, directory: str, package_name: str, version: typing.Optional[str] = None
        ...
        local_maven_repo = os.path.expanduser("~/.m2/repository")
        pom_path_in_repo = self._get_path_for_artifact(
            package_name, version, classifier=None, extension="pom"
        )
        full_pom_path = os.path.join(local_maven_repo, pom_path_in_repo)

        if not os.path.exists(full_pom_path):
            raise FileNotFoundError(
                f"Could not find pom for {package_name}:{version} in local Maven repository. "
    ...
    def get_local_package_info(self, path: str) -> tuple[dict, str]:
        ...
        if jar_files:
            ...
            if not package_info:
                ...
                package_info = {"info": {"name": jar_name, "version": "unknown"}}
                log.warning(
                    f"Could not find POM information for {jar_file}, using filename as package name"
                )
